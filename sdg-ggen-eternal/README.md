...
- 標籤視窗（ui.js `openTagModal`，單位與角色共用）採**暫存 → 套用**制。「💾 套用」才寫回並 `fc(t)`；關閉／Esc／點背景＝捨棄。
  - 左欄是對齊清單：勾選＝包含，右側「排除」＝隱藏具備該標籤的資料（同一標籤不可同時包含與排除），欄位為名稱／使用筆數／排除。可搜尋名稱、全選目前符合項目、清空。
  - 右欄分成上下兩區「包含」與「排除」，各顯示筆數；沒有選項時各區有自己的空狀態。列上「移除」只改暫存。
